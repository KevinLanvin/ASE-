...
void read_inode (unsigned int inumber, struct inode_s *inode){
	read_block_n(current_volume,inumber,(unsigned char*)inode,sizeof(struct inode_s));
}

void write_inode (unsigned int inumber, const struct inode_s *inode){
	write_block_n(current_volume,inumber,(unsigned char*)inode,sizeof(struct inode_s));
}
...
int delete_inode(unsigned int inumber){
	...
	if(inode.ind_indirect != BLOC_NULL){
		...
		read_block_n(current_volume,inode.ind_indirect,(unsigned char*)&indirect,(unsigned int)(NBPB*sizeof(unsigned int)));
	...
	if(inode.ind_2indirect != BLOC_NULL){
		...
		read_block_n(current_volume,inode.ind_2indirect,(unsigned char*)&dindirect,(unsigned int)(NBPB*sizeof(unsigned int)));
		unsigned int i;
		for(i=0;i<NBPB;++i){
			if(dindirect[i] != BLOC_NULL){
				unsigned int ddindirect[NBPB];
				read_block_n(current_volume,dindirect[i],(unsigned char*)&ddindirect,(unsigned int)(NBPB*sizeof(unsigned int)));
